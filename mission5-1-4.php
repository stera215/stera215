...
<body>
    ...
    <!--でないと、画面開いた時にいちいちnoticeとかでたり、入力欄上に<br>がでてきてしつこいです-->
     ...
     <form action="" method="post">
        <input type="text" name="name" placeholder="名前" value=<?php if(!empty($_POST["editnum"])){if($editid == $edit && $editpassword == $edpass){ echo $editname;}}?>> <br>
        <input type="text" name="comment" placeholder="コメント" value=<?php if(!empty($_POST["editnum"])){if($editid == $edit && $editpassword == $edpass){ echo $editcomment; }}?>><br>
        <input type="text" name="pass" placeholder="パスワード"value=<?php if(!empty($_POST["editnum"])){if($editid == $edit && $editpassword == $edpass){ echo $editpass;}} ?>>
